	...
	<form:form action="modifyDoctor" id="doctor_modify" modelAttribute="doctorVO">
		...
		<ul>
			...
			<li>
			    ...
			    <input type="search" name="keyword" id="keyword" value="${hospital.hos_name}">
			...
			<li style="margin-left:205px;">
			    <form:select path="hos_num">
			        <c:forEach var="hos" items="${hosList}">
			            <option value="${hos.hos_num}" <c:if test="${hos.hos_num eq hospital.hos_num}">selected</c:if>>${hos.hos_name}/${hos.hos_addr}</option>
			        </c:forEach>
			    </form:select>
			</li>
            <li>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        // 선택된 병원 정보를 숨겨진 input에 설정
                        var selectedHosNum = $('form').find('select[name="hos_num"]').val();
                        $('form').find('input[name="hos_num"]').val(selectedHosNum);
