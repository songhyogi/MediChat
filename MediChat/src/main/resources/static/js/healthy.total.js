 $(function(){
			...
			success:function(param){
					...
					$(param.list).each(function(index,item){
						...
						if(param.user_num == item.mem_num){
							output+='<div class="align-right btnspace"><input type="button" data-content="'+item.hre_content+'" data-num="'+item.hre_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn"><input type="button" value="삭제" data-num="'+item.hre_num+'" data-mem="'+item.mem_num+'" class="delete-btn"></div>';	
						}
						output +='</div>';
						...
						output+='<input  class="checkbox" type="checkbox" checked="checked">';}
						else{
						...
						output +='<span class="hrefavcount">'+item.refav_cnt+'</span>';
						output +='</div>';
	...
	//댓글 좋아요
	$(document).on('click','.hrefav',function(){
					...
					$.ajax({
						...
						success:function(param){
							if(param.result =='logout'){
								...
								if(param.count == null){
									$(this).find('.hrefavcount').text(0);
								}else{
									$(this).find('.hrefavcount').text(param.count);
								}
