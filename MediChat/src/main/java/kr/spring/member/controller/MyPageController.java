		...
		//로그 처리
		    ...
		    public String form(Model model, HttpSession session) {
		        ...
		        if (user != null) {
		            ...
		            doctorVO.setDoc_email(user.getDoc_email());
		        }
		        if(user == null) {
			...
			public String submit(@Valid DoctorVO doctorVO, BindingResult result, HttpServletRequest request, Model model, HttpSession session) {
			    ...
			    doctorVO.setDoc_num(doctor.getDoc_num());

			    // 현재 로그인한 사용자의 정보 가져오기
			    DoctorVO db_doctor = doctorService.selectDoctor(
	                     doctorVO.getDoc_num());
				/* DoctorVO loggedInDoctor = doctorService.selectDoctor(user.getMem_num()); */
			    
			  //폼에서 전송한 현재 비밀번호와 DB에서 읽어온 비밀번호 일치 여부 체크	
				if(!db_doctor.getDoc_passwd().equals(
						           doctorVO.getNow_passwd())) {
					result.rejectValue("now_passwd", "invalidPassword");
					return "myPage";
				}
				/*
				 * // 입력한 비밀번호와 현재 로그인한 사용자의 비밀번호 비교 if
				 * (!loggedInDoctor.getDoc_passwd().equals(doctorVO.getDoc_passwd())) {
